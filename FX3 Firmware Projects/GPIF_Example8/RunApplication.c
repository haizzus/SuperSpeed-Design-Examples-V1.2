// Demonstrate GPIF as a Master FIFO receiving data from the CPLD
//
// GPIF_Example8 uses GPIF file MasterFifoReadWrite.h and CPLD requires CPLDasFifoSlave.xsvf
// Preset Button[6] = 1 to enable READ from CPLD
//
...
//	2	On a READ the CPLD produces a LastRDData signal - this is detected by the GPIF state machine which COMMITs the last short packet
//		This enables the DMA channel to be set up as AUTO so no CPU attention is required
//  3	I was incorrectly using DebugPrint in several CallBack routines - I now set an Event and use DebugPrint in Main context
//	4	The GPIF State machine needed to be expanded since the GPIO's take TWO clock cycles to reach the output pins
//		The CPLD needs one clock to process the inputs and generate outputs, so the GPIF SM must request GPIO_RD asserted 3 clocks before IN_DATA
//		We will read and later write from/to the CPLD so I added an OE signal for the counter.
//		The CPLD tri-state to valid output delay requires that OE be driven one clock sooner than RD
//	5	If a USB 2.0 connection is made then I slow the GPIF clock down by x10 else it appears that the example is not running - it is but 10x slower!
//
...
void CyFxApplicationDefine(void)
{
    ...
    Status = CyU3PThreadCreate(&ThreadHandle, 	// Handle to my Application Thread
            "11:GPIF_Example8",               	// Thread ID and name
