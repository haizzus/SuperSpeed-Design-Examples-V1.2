...
extern CyBool_t I2C_DebugEnabled;       // Debug Init has been called or not
...
{
	if (glDebugTxEnabled || I2C_DebugEnabled)				// Need to wait until ConsoleOut is enabled
	{
...
uint32_t Swap4Bytes(uint32_t Value)
{
	union { uint32_t D; uint8_t B[4]; } Temp;
	uint8_t TempByte;
//	DebugPrint(4, "\r\nSwapIn = %X", Value);
	Temp.D = Value;
	TempByte = Temp.B[3];
