//	Demonstrate Dual Channel GPIF sending CPLD data to USB
...
// It also needs to know which speed USB is running at (since DMA buffer sizes change)
{
...
void ApplicationThread(uint32_t Value)
{
    ...
    if (Status == CY_U3P_SUCCESS)
    {
		...
		{
			// Check for USB CallBack Events every at least every 100msec
...
void CyFxApplicationDefine(void)
{
    ...
    Status = CyU3PThreadCreate(&ThreadHandle, 	// Handle to my Application Thread
            "11:GPIF_Example3",               	// Thread ID and name
