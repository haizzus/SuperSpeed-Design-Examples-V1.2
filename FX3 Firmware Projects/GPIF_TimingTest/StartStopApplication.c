...
#include "TimingTest.h"				// File generated by GPIF Designer
char* CyFxGpifConfigName = { "TimingTest" };
...
extern CyBool_t glChannelSuspended;
...
void DualGpifToUsbDmaCallback (CyU3PDmaMultiChannel *chHandle, CyU3PDmaCbType_t type, CyU3PDmaCBInput_t *input)
{
	glCounter[0]++;
    if (type == CY_U3P_DMA_CB_CONS_SUSP) glChannelSuspended = CyTrue;
    if (type == CY_U3P_DMA_CB_PROD_EVENT) glCounter[0]++;
...
// USB has been enumerated, time to start the application running
{
    ...
    pibClock.clkDiv = (usbSpeed == CY_U3P_SUPER_SPEED) ? 4 : 40;	//	DEBUG at 10MHz with USB 2.0
    pibClock.clkSrc = CY_U3P_SYS_CLK;	// 400/4 = 100MHz for SuperSpeed, 10MHz for High Speed
	...
	// Start the DMA Channel with transfer size to Infinite and start with PING (Offset = 0)
	Status = CyU3PDmaMultiChannelSetXfer(&glDualGPIF2USB_Handle, 0, 0); //dmaMultiConfig.size, 0);
