// Demonstrate GPIF as a Master FIFO sending data to the CPLD
//
// GPIF_Example9 uses MasterFifoWrite.h and CPLD requires CPLDasFifoSlave.xsvf
// Preset Button[6] = 0 to WRITE to CPLD
//
...
//	2	On a WRITE the FX3 no longer produces a LastWRData signal - this allows the example to be used repeatedly with no resets
//		On a READ the CPLD produces a LastRDData signal - this is detected by the GPIF state machine which COMMITs the last short packet
...
//		The CPLD needs one clock to process the inputs and generate outputs, so the GPIF SM must request GPIO_WR asserted 3 clocks before DR_DATA
...
void CyFxApplicationDefine(void)
{
    ...
    Status = CyU3PThreadCreate(&ThreadHandle, 	// Handle to my Application Thread
            "11:GPIF_Example9",               	// Thread ID and name
