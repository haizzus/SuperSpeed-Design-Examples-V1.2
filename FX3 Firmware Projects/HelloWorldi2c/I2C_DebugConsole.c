...
void I2C_ConsoleThread(uint32_t Value)
{
    ...
    while (1)
    {
        Q_Status = CyU3PQueueReceive(&I2C_DebugQueue, &FilledBuffer, 1000);
...
//                CheckStatus("CyU3PDmaChannelWaitForCompletion(RX)", Status);
        ...
        if (Q_Status == CY_U3P_SUCCESS)
        {
            ...
            if (Status != CY_U3P_SUCCESS) CheckStatus("CyU3PDmaChannelSetupSendBuffer", Status);
            // Now tell the I2C Block what to do with this buffer of data
            Preamble.buffer[0] &= 0xFE;	// Clear LSb = Write
            if (Status == CY_U3P_SUCCESS) Status = CyU3PI2cSendCommand(&Preamble, FilledBuffer.count, CyFalse);
            ...
            if (Status != CY_U3P_SUCCESS)
            {
                ...
                if (retryCount > 0)
                {
                    ...
                    if (Status != CY_U3P_SUCCESS) CyU3PDebugPrint(4, "Unable to re-queue data");
                }
            }
