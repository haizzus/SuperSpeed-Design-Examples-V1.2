...
#define     PACKETS_PER_TRANSFER                256
...
CSimpleArray<int> deviceIndexRelation;
...
BEGIN_MESSAGE_MAP(CFx3ReceiveDataDlg, CDialogEx)
    ...
    ON_BN_CLICKED(IDOK, &CFx3ReceiveDataDlg::OnBnClickedOk)
END_MESSAGE_MAP()
...
BOOL CFx3ReceiveDataDlg::SurveyExistingDevices()
{
    ...
    if (USBDevice != NULL) 
    {
        ...
        for (int nCount = 0; nCount < nDeviceCount; nCount++ )
        {
            ...
            if (USBDevice->bHighSpeed || USBDevice->bSuperSpeed ) 
            {
                ...
                deviceIndexRelation.Add(nCount);
            }
            USBDevice->Close();
        }
        delete USBDevice;
...
LRESULT CFx3ReceiveDataDlg::DataTransferComplete(WPARAM wParam, LPARAM lParam)
{
    ...
    SetEvent(m_hDataQueueEvent);
    if (m_pInfoMessage ) 
            m_pInfoMessage->PostMessage(WM_CLOSE, 0, 0);     
    GetDlgItem(IDC_BTN_FILE)->EnableWindow(TRUE);
...
LRESULT CFx3ReceiveDataDlg::DataWriteRoutine(LPVOID lpParam)
{
    ...
    if (pWorkingSet == NULL ) 
    {
        ...
        do {
            ...
            if (pThis->m_bBreakStreamerThreadCollection == TRUE ) break;
        } while(pHead == NULL);

        // Wait till the two datas are available to write.
        if (pHead && pHead->pNextData == NULL )
            WaitForSingleObject(pThis->m_hDataQueueEvent, INFINITE);
...
LRESULT CFx3ReceiveDataDlg::PerformStreamerDataCollection(LPVOID lpParam)
{
    ...
    ////////////////////////////////////////////////////////////
    if (USBDevice->Open(deviceIndexRelation[nDeviceIndex]) == false ) 
    {
    ...
    //isItSuperSpeed = (USBDevice->bSuperSpeed || USBDevice->bHighSpeed);    
    ...
    if (pThis->GetDlgItem(IDC_CBO_TIMEOUT)->IsWindowEnabled() == TRUE )
    {
        ...
        double nElapseTime = _wtof(strData.GetBuffer(0));
        // Ontimer callback function will be called at the time out.
        pThis->SetTimer(STOP_DATA_COLLECTION_EVENT, (int)(nElapseTime*1000.00), NULL);
    }
    ...
    while ( pThis->m_bBreakStreamerThreadCollection == FALSE )
    {
        ...
        DWORD dwWrite = 0;
        if (pThis->m_hFileData != INVALID_HANDLE_VALUE && readLength > 0 ) 
        {
            ///////////File writting is valid, so push the data to the file.////////////
            //// Is this device super speed device?
            if ((USBDevice->bSuperSpeed || USBDevice->bHighSpeed) == TRUE )
            {
                ///////////////////////////////////////////////////////
        ...
        if (ept->NtStatus || ept->UsbdStatus) 
        {
            if (ept->UsbdStatus == USBD_STATUS_DEVICE_GONE ) break;
            // BeginDataXfer failed............
        ...
        {
            nCount = 0;
            pThis->CalculateTransferSpeed(objStartTime, BytesXferred);
        }
    }
...
void CFx3ReceiveDataDlg::OnTimer(UINT_PTR nIDEvent)
{
    ...
    {
        PostMessage(WM_DATA_TRANSFER_COMPLETE, 0, 0);
        KillTimer(nIDEvent);
    }
...
void CFx3ReceiveDataDlg::OnBnClickedOk()
{
